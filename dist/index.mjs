...
function readHeader(req, name) {
  var _a, _b, _c, _d;
  const direct = ((_a = req.header) == null ? void 0 : _a.call(req, name)) ?? ((_b = req.get) == null ? void 0 : _b.call(req, name));
  if (direct) {
    return direct;
  }
  const lowerName = name.toLowerCase();
  const value = ((_c = req.headers) == null ? void 0 : _c[lowerName]) ?? ((_d = req.headers) == null ? void 0 : _d[name]);
...
async function extractChallengeResult(req, handlers) {
  var _a, _b;
  const proofToken = readHeader(req, "x-txnshield-proof-token") ?? (typeof ((_a = req.body) == null ? void 0 : _a.challengeProofToken) === "string" ? req.body.challengeProofToken : void 0);
  if (proofToken && (handlers == null ? void 0 : handlers.verifyProofToken)) {
    return {
      type: "proof_token",
      passed: await handlers.verifyProofToken(proofToken)
    };
  }
  const passkeyAssertion = readHeader(req, "x-txnshield-passkey-assertion") ?? (typeof ((_b = req.body) == null ? void 0 : _b.passkeyAssertion) === "string" ? req.body.passkeyAssertion : void 0);
  if (passkeyAssertion && (handlers == null ? void 0 : handlers.verifyPasskeyAssertion)) {
    return {
      type: "passkey",
  ...
  const protect = (definition) => {
    return async (req, res, next) => {
      ...
      try {
        ...
        const session = ((_a = definition.session) == null ? void 0 : _a.call(definition, req)) ?? {
        ...
        const evaluation = await evaluate({
          ...
          requestData: ((_b = definition.requestData) == null ? void 0 : _b.call(definition, req)) ?? {},
          session,
          metadata: ((_c = definition.metadata) == null ? void 0 : _c.call(definition, req)) ?? {
            requestedCount: numberFrom(((_d = req.body) == null ? void 0 : _d.requestedCount) ?? ((_e = req.query) == null ? void 0 : _e.requestedCount)),
            velocityWindowCount: numberFrom(((_f = req.body) == null ? void 0 : _f.velocityWindowCount) ?? ((_g = req.query) == null ? void 0 : _g.velocityWindowCount)),
            changedFields: Array.isArray((_h = req.body) == null ? void 0 : _h.changedFields) ? req.body.changedFields.map(String) : []
          },
          challengeResult,
          rawSignals: {}
        });
